/**
 * ====================================================================
 * MOTOR DE BATALHA: BATTLE ENGINE V1 + TYPE SYSTEM (battle-engine.js)
 * ====================================================================
 * Núcleo matemático de simulação de combate 1x1 das Fases PBA-003 e PBA-004.
 * 
...
(function () {
  ...
  const BattleEngine = (() => {
    ...
    /**
     * Valida e normaliza um Pokémon para o modelo de combate v2.
     * Não muta o objeto de entrada.
     * 
     * @param {Object} raw - Dados brutos do Pokémon.
     * @returns {Object} Combatente normalizado com atributos e tipos validados.
    ...
    function createCombatant(raw) {
      ...
      // Validação e normalização dos tipos (Fase PBA-004)
      ...
      // Utiliza TypeEffectiveness para normalizar e garantir 1 ou 2 tipos válidos sem duplicatas
     ...
     * 1. ACTION_STARTED
     * 2. TYPE_EFFECTIVENESS_RESOLVED
     * 3. DAMAGE_APPLIED
     * 4. POKEMON_FAINTED (se currentHp === 0)
     * 5. BATTLE_ENDED (se currentHp === 0)
     * 
     * @private
    ...
    function executeAction(attackerRole, defenderRole, state, events) {
      ...
      // Regra temporária da Fase PBA-004: BASIC_ATTACK usa o tipo primário do atacante
      ...
      // 2. Calcula a efetividade de tipos (TypeEffectiveness)
      const effectiveness = TypeEffectiveness.calculate(attackType, defender.types);

      // 3. Emite evento estruturado de efetividade
      events.push({
      ...
      // 4. Calcula dano base e dano final com modificador
      const baseDamage = DamageCalculator.calculateBaseDamage(attacker.attack, defender.defense);
      const finalDamage = DamageCalculator.applyModifier(baseDamage, effectiveness.multiplier);

      // 5. Aplica o dano respeitando o piso de 0 HP (ou 0 dano se imune)
      const previousHp = defender.currentHp;
      const currentHp = Math.max(0, previousHp - finalDamage);
      defender.currentHp = currentHp;

      // 6. Emite evento de dano aplicado com dados completos
      events.push({
      ...
      // 7. Verifica se o defensor foi nocauteado (apenas se dano > 0)
      if (currentHp === 0) {
        events.push({
