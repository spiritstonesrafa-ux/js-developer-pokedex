(function () {
  ...
  function renderTrainerAvatar(descriptor, options = {}) {
    ...
    const avatarSrc = typeof descriptor.avatarSrc === 'string' && descriptor.avatarSrc.trim() ? descriptor.avatarSrc.trim() : null;
    ...
    const image = avatarSrc ? '<img class="trainer-avatar__image" src="' + escape(avatarSrc) + '" alt="' + label + '" width="' + SIZES[size] + '" height="' + SIZES[size] + '" loading="' + (options.loading === 'eager' ? 'eager' : 'lazy') + '" onerror="this.hidden=true;this.nextElementSibling.hidden=false;this.parentElement.classList.add(&quot;trainer-avatar--image-failed&quot;)">' : '';
